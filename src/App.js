...
const App = () => {
	...
	const [ basketSettings, setBasketSettings ] = useState(JSON.parse((localStorage.getItem('basketSettings') || 'null')) || {});
			...
			<Switch>
				...
				<Route path="/basket/:areaId/:itemId" exact>
					<Basket
						...
						addBasketSettings = {( areaId , settings ) => {
							const nextBasketSettings = {...basketSettings};
							
							nextBasketSettings[areaId] = settings;
							setBasketSettings(nextBasketSettings);

							localStorage.setItem('basketSettings', JSON.stringify(nextBasketSettings));
						}}
						unsetBasketSettings = {( areaId ) => {
							const nextBasketSettings = {...basketSettings};

							delete nextBasketSettings[areaId];
							setBasketSettings(nextBasketSettings);
