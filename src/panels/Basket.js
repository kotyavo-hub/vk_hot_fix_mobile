...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order, basketSettings, addBasketSettings, unsetBasketSettings}) => {
  const [ faster, setFaster ] = useState((basketSettings[areaId]) ? basketSettings[areaId].faster : true);
  const [ time, setTime ] = useState((basketSettings[areaId]) ? basketSettings[areaId].time : '');
  const [ selfService, setSelfService ] = useState((basketSettings[areaId]) ? basketSettings[areaId].selfService : false);
  ...
  const orderButton = () => { 
    if (products && Object.keys(products).length) { //Object.keys(order).length
      return (
        <Link 
          to={`/order/${area.id}/${item.id}`}
          className="Place__order"
          onClick={(e) => {
            unsetBasketSettings(areaId);
          }}
        >
          Оплатить {price}
        </Link>
      ...
      return (
        <span className="Place__order-empty">
          Оплатить
        </span>
      )
    }
